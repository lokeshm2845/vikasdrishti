...
const GeofenceMap = ({ leaderData }) => {
    ...
    return ( <
        div style = {
            { height: '100vh', display: 'flex', flexDirection: 'column' } } > { /* Header */ } <
        div style = {
            {
                background: '#FF9933',
                padding: '15px 20px',
        ...
        <
        h2 style = {
            { margin: 0 } } > Leader Dashboard < /h2> {
            leaderData && ( <
                p style = {
                    { margin: '5px 0 0' } } > { leaderData.title || '' } | { leaderData.constituency || '' } <
                /p>
            )
        } <
        /div>
        ...
        div style = {
            { display: 'flex', flex: 1, overflow: 'hidden' } } > { /* Left Panel - Map */ } <
        div style = {
            { flex: 2, display: 'flex', flexDirection: 'column' } } > { /* Map Controls */ } <
        div style = {
            { padding: '10px', background: '#f8f9fa', borderBottom: '2px solid #138808' } } >
        <
        button onClick = {
            () => setDrawing(!drawing) }
        style = {
        ...
        { /* Project Details */ } <
        div style = {
            { padding: '10px', background: '#e9ecef', display: 'flex', gap: '10px' } } >
        <
        ...
        <
        h3 style = {
            { margin: '0 0 15px 0', color: '#FF9933' } } > 📋Pending Complaints <
        /h3>

        {
            complaints.length === 0 ? ( <
                p style = {
                    { color: '#666', textAlign: 'center' } } > No complaints yet < /p>
            ) : (
                complaints.filter(c => c.status !== 'resolved').map(complaint => ( <
                    ...
                    style = {
                        {
                            ...
                            border: complaint.status === 'pending' ? '2px solid #dc3545' : complaint.status === 'in_progress' ? '2px solid #ffc107' : '2px solid #28a745'
                        }
                    } >
                    <
                    div style = {
                    ...
                    <
                    h4 style = {
                        { margin: '0 0 10px 0' } } > { complaint.title } < /h4> <
                    span style = {
                        {
                            padding: '3px 8px',
                            borderRadius: '12px',
                            fontSize: '12px',
                            background: complaint.status === 'pending' ? '#dc3545' : complaint.status === 'in_progress' ? '#ffc107' : '#28a745',
                            color: 'white'
                        }
                    } > { complaint.status } <
                    /span> <
                    ...
                    <
                    p style = {
                        { fontSize: '14px', color: '#666' } } > { complaint.description } < /p>

                    {
                        complaint.photo_url && ( <
                            ...
                            alt = "Complaint"
                            style = {
                                { width: '100%', maxHeight: '150px', objectFit: 'cover', borderRadius: '5px', margin: '10px 0' } }
                            />
                        )
                    }
