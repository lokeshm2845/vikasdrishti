...
import { FaMapMarkedAlt, FaList, FaUser, FaSignOutAlt, FaBell } from 'react-icons/fa';
...
const LeaderDashboard = () => {
    ...
    const loadComplaints = async () => {
        ...
        if (result.success) {
            ...
            const inProgress = result.data.filter(c => c.status === 'in_progress').length;
            const resolved = result.data.filter(c => c.status === 'resolved').length;
